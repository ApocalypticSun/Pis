...
class ImageSegmenter:
    ...
    def region_based_segmentation(self, arg_type, threshold=128, block_size=11, constant=2):
        if arg_type == "binary":
            ...
            binary = cv2.adaptiveThreshold(self.gray,
                                           255,
    ...
    def watershed_segmentation(self, gaussian_ksize=5, binary_thresh=0, opening_iter=2, dilate_iter=3,
                               dist_thresh=0.7, kernel_size=3):
...
class SegmentationGUI:
    def __init__(self, arg_root):
        self.root = arg_root
        self.root.configure(bg=THEME_COLOR_LIGHT)  # Bright pink background
        self.segmenter = None
        self.current_image = None

        style = ttk.Style()

        # Styling of the background area that doesn't include the image area
        style.configure('TFrame', background=THEME_COLOR_LIGHT)

        # Styling of the text labels
        # - 'background' represents the color behind the label,
        #    as well as the color of the background area that includes the image area
        # - 'foreground' represents the color of the label's text
        style.configure('TLabel', background=THEME_COLOR_LIGHT, foreground='black', font=('Arial', 10, 'bold'))

        # Styling of the buttons
        # - 'background' represents the color behind the button, NOT the button's color
        # - 'foreground' represents the color of the button's text
        style.configure('TButton', background=THEME_COLOR_LIGHT, foreground='black', font=('Arial', 10, 'bold'))

        # Styling of the ???
        # - 'background' represents the color ???
        # - 'foreground' represents the color ???
        style.configure('TLabelFrame', background=THEME_COLOR_LIGHT, foreground='black', font=('Arial', 10, 'bold'))

        # Styling of the background color for the sliders
        style.configure('TScale', background=THEME_COLOR_LIGHT)

        # Styling of the button where you select the segmentation method
        # - 'background' represents the button's color
        # - 'foreground' represents the color of the button's text
        ...
        if file_path:
            try:
                if self.segmenter is None:
                    ...
                    self.segmenter.__init__(file_path)
                self.reset_params()
                self.update_image()
            except Exception as e:
    ...
    def update_controls(self):
        ...
        if method == "binary":
            ...
            ttk.Label(self.params_frame,
                      text="Otsu's method uses automatic thresholding.\nNo parameters required.").pack()
    ...
    def add_slider(self, name, min_val, max_val, default_val, step=1):
        ...
        slider = ttk.Scale(frame,
                           ...
                           command=lambda v,
                           tv=current_val,
                           s=step: tv.set(str(int(float(v))))
                           if s == 1
                           else tv.set(str(int(float(v)) // 2 * 2 + 1)), length=150)
        ...
        try:
            ...
            if segmented.ndim == 2:
                if segmented.dtype == np.int32:
                    segmented = cv2.normalize(segmented,
                                              None,
                                              0,
                                              255,
                                              cv2.NORM_MINMAX,
